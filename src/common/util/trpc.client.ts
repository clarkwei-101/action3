...
import { cloudTransformerClient } from '~/common/util/cloud-transformer';
...
/** Typesafe async/await hooks for the the Node functions API */
export const apiAsyncNode = createTRPCClient<AppRouterCloud>({
  links: [
    loggerLink({ enabled: enableLoggerLink }),
    httpLink({
      url: `${getBaseUrl()}/api/cloud`,
      transformer: cloudTransformerClient,
    }),
  ],
});

/** Typesafe React Query hooks for the tRPC Cloud-Runtime API (used by Action3) */
