...
function deserializeWithWorkaround<T = unknown>(object: unknown): T {
  ...
  // special instruction for the `json` object.
  const obj = object as Record<string, unknown> | null;
  if (obj && typeof obj === 'object' && obj.meta && obj.json)
    object = { ...obj, json: shallowClone(obj.json) };

  return superjson.deserialize<T>(object as Parameters<typeof superjson.deserialize>[0]);
}
